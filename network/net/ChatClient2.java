...
implements ActionListener, Runnable {
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if(obj ==bt4 || obj ==tf3)//sned
		{
			String str = tf3.getText();
			if(filterMgr(str))
			{
				new MDialog(this, "경고", "입력하신 글짜는 금지어입ㄴ디ㅏ.");
				return;
			}
			if(!flag)//아이디 입력
			{
			...
			{
				int idx= list.getSelectedIndex();//순서값
				if(idx==-1|| idx==0)
				{
				...
				{
					String id=list.getSelectedItem();//id?값 그자체
	...
	public void routine(String line) 
	{
		...
		if(cmd.equals(ChatProtocol2.CHATLIST))//data=aa;bbb;ccc;
		{
			list.removeAll();//기존 리스트 삭제
			list.add(listTitle);
		...
		{
			area.append(data+"\n");//CHAT:[aaa(S)]메세지 &CHATALL:[aaa(S)]메세지
		}
	...
	public static void main(String[] args)
	{
		new ChatClient2();
		/*
		 * String str="aaa;bbb;ccc;홍길동;"; StringTokenizer st= new
